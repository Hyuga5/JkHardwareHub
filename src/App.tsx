...
import { DistributorDealerList } from './components/distributor/DistributorDealerList';
import { DistributorOrderManager } from './components/distributor/DistributorOrderManager';
import { DistributorStockInventory } from './components/distributor/DistributorStockInventory';
import { DistributorAccountingSuite } from './components/distributor/DistributorAccountingSuite';
...
function MainApp() {
  ...
  const [shopTab, setShopTab] = useState<
    'dashboard' | 'accounting' | 'orders' | 'catalog' | 'boosting' | 'kyc'
  >('dashboard');
  const [distributorTab, setDistributorTab] = useState<
    | 'dashboard'
    | 'dealers'
    | 'orders'
    | 'stock'
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900 flex flex-col font-sans selection:bg-orange-500 selection:text-white">
        ...
        {currentRole === 'distributor' && (
          <>
            {distributorTab === 'dashboard' && (
              <DistributorDashboard onNavigate={setDistributorTab} />
            )}
            {distributorTab === 'dealers' && (
              <DistributorDealerList
                onNavigateToAccounting={() => setDistributorTab('accounting')}
              />
            )}
      ...
      {/* Bottom Sticky Status / Compliance Indicator */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <span className="text-slate-500 hidden sm:inline">
              • Inland Revenue Department (IRD) 13% Tax & BusyWin Architecture
            </span>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-slate-500">Kathmandu • Lalitpur • Pokhara • Birgunj</span>
            <span className="text-orange-600 font-mono font-bold bg-orange-50 px-2 py-0.5 rounded-md border border-orange-200">
              BS 2081/82
            </span>
          </div>
        </div>
